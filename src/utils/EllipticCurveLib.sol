...
// Inspired by witnet/elliptic-curve-solidity - MIT licence
...
library EllipticCurveLib {
    ...
    function validateSignature(uint256 h, uint256 pgx, uint256 pgy, uint256 r, uint256 s)
        internal
        pure
        returns (bool)
    {
        if (!isOnCurve(pgx, pgy, a, b, p)) {
            return false;
        }
        uint256 s1 = invMod(s, n);
        (uint256 _x, uint256 _y) = ecMul(mulmod(h, s1, n), gx, gy, a, p);
        (uint256 _xx, uint256 _yy) = ecMul(mulmod(r, s1, n), pgx, pgy, a, p);
        (uint256 Rx,) = ecAdd(_x, _y, _xx, _yy, a, p);
        return Rx == r;
    }
}
